 ...
 *
 * W przypadku dodawania elementów do książki telefonicznej opracowaliśmy funkcje pozwalająca na wygenerowanie najmniejszego unikatowego numeru id i przypisanie go do nowo utworzonego elementu. Dzięki takiemu rozwiązaniu nie ma później problemu z wyborem elementu po ID do edycji lub usunięcia.
 *
 * Wyszukiwanie elementów opracowaliśmy poprzez tworzenie tymczasowej listy, do której zostaja skopiowane elementy spełniające warunek wyszukiwania. Porównanie elementów jak przy sortowaniu listy zostało uzależnione od wskaźnika na funkcję, dzięki czemu można przeszukać książkę po różnych elementach wpisu.
 *
 * Ostatnim aspektem książki telefonicznej jest możliwość zapisu i odczytu danych z pliku. Jako folder gdzie będą przechowywane dane o książce telefonicznej utworzyliśmy folder “phone_data” a jako format pliku “.pbd”. Aby ułatwić użytkownikowi wybór pliku i uniknięcie próby otworzenia pliku, który nie istnieje wykorzystaliśmy biblioteke dirent.h do wyświetlenia listy plików w folderze phone_data i następnie możlwość wybrania numeru pliku i jego otwarcie.
 *
 *
 * Wszystkie nasze cele udało nam się wykonać na pierwszy termin, dzięki czemu na drugi termin pozostało nam jedynie poprawienie ewentualnych błędów w programie czy też poprawnie błędów pisowni, a także opracowanie dokumentacji doxygen.
 *
 *
 * \section autorzy
 *
