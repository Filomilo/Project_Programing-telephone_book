 ...
 * @brief plik zawiera funkcje odpowiedzialne za konsolowy interfejs użytkownika
 * @version 0.1
 * @date 2022-03-24
 * 
...
#include <string.h>
 ...
 /**
  * @brief funkcja odpowiedzialna za wpisywanie nowego elementu do listy o podanych przez użytkownika danych
  *
  * funkcja tworzy nowy element contact_type a następnie po kolei prosi użytkownika o podanie:
  * imienia i nazwiska, nazwy ulicy, numeru domu/budynku, kodu pocztowego, nazwy miasta, numer telefonu,
  * następnie element_contact_type zostaje dodany do listy z użyciem funkcji list_add_head
  *
 ...
 *
 * funkcja najpierw prosi użytkownika o podanie ciągu znaków według której ma zostać przeszukana lista,
 * a następnie tworzy listę rezultatów z użyciem tego ciągu znaków oraz funkcji podanej jako argument funkcji określający
 * według którego elementu jest przeszukiwana
 *
 * @param list - struktura list_pointers określająca listę która użytkownik przeszukuje
 * @param search_option - wskaźnik na funkcje określający według którego elementu kontaktu będzie można wybrać przeszukiwanie:
 ...
 * w zależności od wyboru funkcja później przechodzi do funkcji searching z odpowiednio wybrana funkcja "is_searching_******"
 *
 *
 * @param list - struktura list_pointers określająca listę która użytkownik przeszukuje
 */
 ...
 *
 * funkcja na początku wyświetla uzytkownikowi opcje wyboru według którego elementu użytkownik chce posortować listę,
 * w zależności od wyboru użytkownika uruchamia się funkcja merge_sort ze wskaźnikiem na funkcje "****_cmp",
 * na koniec zwraca posortowana listę do głównego programu
 *
 * @param list -  struktura list_pointers okreslajaca listę która użytkownik chce posortować
 ...
 * @brief funkcja generuje interfejs umożliwiający sortowanie listy
 *
 * funkcja na początku wyświetla użytkownikowi opcje wyboru według którego elementu użytkownik chce posortować listę,
 * w zależności od wyboru użytkownika uruchamia się funkcja merge_sort ze wskaźnikiem na funkcje "****_cmp",
 * na koniec zwraca posortowana listę do głównego programu
 *
 * @param list -  struktura list_pointers okreslajaca listę która użytkownik che posortować
 * @param order - wskaźnik na zmienna order określajaca w jakim kierunku ma się wyświetlać lista
 * @return list_pointers -  struktura list_pointers określająca posortowaną listę w zależności od wyboru użytkownika
 */
 ...
 *
 * @param list  struktura list_pointers określająca listę z której użytkownik ma usunąć element
 * @param order - zmienna order określajaca w jakim kierunku ma się wyświetlać lista
 ...
 *
 * na początku funkcja pyta sie użytkownika o podanie ciągu znaków określającego nazwę pliku pod jaką ma być zapisany,
 * a następnie zapisuje ta liste pod tą nazwę z użyciem funkcji save()
 *
 * @param list  struktura list_pointers określająca listę którą użytkownik chce zapisać
 */
...
/**
 * @brief funkcja generuje interfejs umożliwiający edycje konkretnego elementu wpisu w książce telefonicznej
 *
...
void edit_element(list_node* element_to_edit)
{
	...
	printf("\n");
		...
		int choice;
	do {
		printf("wWhat element would you like to edit: \n\n");
		printf("7- edit name\n6- edit surname\n5- edit street name\n4- house number\n3- edit postal code\n2- edit city\n1- edit phone number\n0- finish editing this contact\n\n ");
		scanf_s("%d", &choice, 1);
		if (choice == 0)
			break;
 ...
 * następnie przeszukuje listę w jego poszukiwaniu, jeżeli nie ma elementu o takim id funkcja wyświetla że nie znaleziono takiego
 * elementu, po znalezieniu elementu funkcja aktywuje funkcję edit_element na wybranym elemencie listy. Po zakończeniu funkcja zwraca\
 * zedytowana listę
 *
 * @param list  struktura list_pointers określająca listę którą użytkownik chce zapisać
...
list_pointers edit_phone_list(list_pointers list)
{
	...
	do
	{
		...
		printf("\n\n Would you like to edit another element: \n 1- yes \n 0- no \n\n");
		scanf_s("%d", &choice, 1);
		if (choice == 0)
			break;
		if (choice == 1)
 ...
 * po zakończeniu pracy funkcja zwalnia pamięć zajmowana przez listę
 *
...
void phone_book_manage(list_pointers list)
{
	...
	do {
		...
		printf("6- add new element\n5- delete element\n4- edit element\n3- sort phone book \n2- search for element\n1- save phone book\n0- finish working with this phone book\n\n ");
		scanf_s("%d", &choice, 1);
		switch (choice)
		{
			case 0:  break;
 ...
 * funkcja tworzy nowa pustą listę a następnie prosi użytkownika o podanie elementu z pomocą funkcji add_new_element
 * az użytkownik nie zdecyduje że chce przestać, po zakończeniu wpisywania przechodzi z ta lista do interfejsu zarządzania
 * phone_book_manage()
 *
 */
 ...
 *
 * funkcja wyświetla użytkownikowi listę książek dostępnych w dedykowanym folderze
 * a następnie pozwala użytkownikowi wybrac numer pliku który chce otworzyć,
...
void opening_book()
{
	...
	if (!phone_book_list())
		return;
	printf("\n\n Please choose number of phone book you would like to open: \n");
	scanf_s("%d", &choice, 1);
	char file_name[30] = "";
